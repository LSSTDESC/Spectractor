...
class Image():
    ...
    def compute_statistical_error(self):
        ...
        # notice units are in ADU/sec
        data = np.copy(self.data)
        zeros = np.where(data<=0)
        min_noz = np.min(data[np.where(data>0)])
        data[zeros] = min_noz
        # compute photoelectron poisson noise
    ...
    def extract_spectrum_from_image(self,w=3,ws=[8,30],right_edge=1800,meanflag=False,filterstarflag=False):
        """
...
#def Spectractor(filename,outputdir,guess,target,atmospheric_lines=True,w=3,ws=[8,30]):
def Spectractor(filename,outputdir,guess,target,atmospheric_lines=True,w=3,ws=[8,30]):
    """ Spectractor
    ...
    print 'w,ws=',w,ws
    spectrum = image.extract_spectrum_from_image(w=w,ws=ws)
...
if __name__ == "__main__":
    ...
    Spectractor(filename,opts.output_directory,guess,target,w=10,ws=[12,30])
