...
#        return [self.data.getentry(c, col) for c in keys]
    ...
    def parsetable(self,headerstring,lines,startindex=0,stopindex=None,addflag=0,skipheaderline=1,skipcomments=1,onlyparse2nextheader=0,maxsplit=0, onlyparsecols=None,noheader=False,comments=None):
        if not addflag:
            self.cleartable()
...
#                    print lineinfo[38]
                    ...
                    raise RuntimeError,'While parsing table: header and line have unequal number of elements: %d != %d !!!!' % (len(headerinfo), len(lineinfo))
...
#                     s+=self.colheader2string(col)
    ...
    def printtxttable(self,file=None,keys=None,cols=None,showallcols=0,showallrows=0,
                      mask='default',col4mask='default',autoformat=1,printheader=20,linelist=None,printheader4nodata=1, verbose=True):
...
#        print 'GGGGGGGGG', type(cols)
            ...
            for key in keys:
                ...
                if sigma>0.0 and Nsigma>0.0:
                    ...
                    if delta * delta > Nsigma * Nsigma * (self.getentry(key,dcol) * self.getentry(key,dcol)):
                        if verbose==1: print "I am getting skipped", abs(delta), Nsigma*self.getentry(key,dcol), (abs(delta)> Nsigma*self.getentry(key,dcol))
                        if verbose>1:
            ...
            for key in keys:
                ...
                if sigma>0.0 and Nsigma>0.0:
                    ...
                    if delta * delta > Nsigma * Nsigma * self.getentry(key,dcol2):
                        if verbose==1: print "I am getting skipped", abs(delta), Nsigma*math.sqrt(self.getentry(key,dcol2)), (abs(delta)> Nsigma*math.sqrt(self.getentry(key,dcol2)))
                        if verbose>1:
    ...
    def calcaverage_errorcutloop(self,keys,col,dcol=None,dcol2 = None,Nsigma=0.0,Nitmax=10,verbose=0):
        ...
        while ((i<Nitmax) or (Nitmax==0)) and (not converged):
            if i==0:
                ...
                Nsigma_temp = Nsigma        
            (mu,mu_err,sigma,X2norm,Nchanged,Nused,Nskipped) = self.calcaverage_errorcut(keys,col,mu,mu_err,sigma,dcol,dcol2,Nsigma=Nsigma_temp,verbose=verbose)
            # Only do a sigma cut if wanted
    ...
    # and put it here
    def selectlckeys(self,keys=None,photcodes=None,dophottypes=None,onlyforced=0,onlynonforced=0,MJDmin=None,MJDmax=None,skipbadim=0,onlybadim=0):
        if keys == None:
            keys = self.rowkeys()
        FORCEDPHOTFLAG = 0x10
    ...
    def straightline_errorcut(self,keys,xcol,ycol,offset,slope,dycol=None,dycol2=None,Nsigma=3.0,dyval=1.0,verbose=0):
            ...
            for key in keys:
                ...
                if Nsigma>0.0:
                    ...
                    if delta * delta > Nsigma * Nsigma * (self.getentry(key,dycol) * self.getentry(key,dycol)):
                        if verbose==1: print "I am getting skipped", abs(delta), Nsigma*self.getentry(key,dycol), (abs(delta)> Nsigma*self.getentry(key,dycol))
                        if verbose>1:
    ...
    def straightline_errorcutloop(self,keys,xcol,ycol,dycol=None,dycol2=None,Nsigma=3.0,dyval=1.0,Nitmax=10,verbose=0):
        ...
        while ((i<Nitmax) or (Nitmax==0)) and (not converged):
            if i==0:
                ...
                Nsigma_temp = Nsigma        
            (slope_b,slope_b_err,offset_a,offset_a_err,Nchanged,Nused,Nskipped) = self.straightline_errorcut(keys,xcol,ycol,offset_a,slope_b,dycol=dycol,dycol2=dycol2,Nsigma=Nsigma_temp,dyval=dyval,verbose=verbose)
            # Only do a sigma cut if wanted
            if verbose>=1 and slope_b!=None and slope_b != None:
                print "slope_b:%f slope_b_err:%f offset_a:%f offset_a_err:%f Nchanged:%d Nused:%d Nskipped:%d" % (slope_b,slope_b_err,offset_a,offset_a_err,Nchanged,Nused,Nskipped)
            if Nsigma == 0.0:
                break
            if (i>0) and (Nchanged==0):
                break
