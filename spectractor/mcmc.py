...
class Chain(txttableclass):
    ...
    def load_and_init(self, filename, createtit=True):
        ...
        if os.path.isfile(covfile):
            ...
            for line in f:
                ...
                if words[0] == 'Parameter':
                    ...
                    self.axis_names.append(words[2])
...
class Chains(Chain):
    ...
    def chains_to_likelihood(self, nchain=-1, pdfonly=False):
        ...
        self.likelihood = Likelihood(centers, labels=self.labels, axis_names=self.axis_names, truth=self.truth)
        for i in rangedim:
    ...
    def convergence_tests(self):
        ...
        for i in range(self.dim):
            ...
            for n in range(len(nchains)):
                ax[i].plot(self.getentries(chain_keys[n], 'Index'), self.getentries(chain_keys[n], self.labels[i]),
                           label='Chain ' + str(nchains[n]))
                ax[i].set_xlabel('Index')
                ax[i].set_ylabel(self.axis_names[i])
        ...
        for n in range(len(nchains)):
            ...
            for i in range(self.dim):
                ...
                ax2[i].plot(self.likelihood.pdfs[i].axe.axis, self.likelihood.pdfs[i].grid, lw=parameters.LINEWIDTH,
                            label='Chain ' + str(nchains[n]))
                ax2[i].set_xlabel(self.axis_names[i])
