...
class Image(object):
    ...
    def find_target_1Dprofile(self, sub_image, guess, rotated=False):
        ...
        if parameters.DEBUG:
            self.my_logger.info('\n\t%d saturated pixels: set saturation level to %d %s.' % (
                len(saturated_pixels[0]), self.saturation, self.units))
        # compute profiles
        ...
        if parameters.DEBUG:
            ...
            self.plot_image_simple(ax1, data=sub_image, scale="log", title="", units=self.units, plot_stats=False,
    ...
    def find_target_2Dprofile(self, sub_image, guess, rotated=False):
        ...
        bounds = [[0.5 * np.max(sub_image_subtracted), avX - mean_prior, avY - mean_prior, 2, 0.9 * self.saturation],
                  [10 * self.saturation, avX + mean_prior, avY + mean_prior, 15, 1.1 * self.saturation]]
        saturated_pixels = np.where(sub_image >= self.saturation)
        ...
        if parameters.DEBUG:
            f, (ax1, ax2, ax3) = plt.subplots(1, 3, figsize=(15, 4))
            self.plot_image_simple(ax1, data=sub_image_subtracted, scale="lin", title="", units=self.units,
                                   target_pixcoords=[new_avX, new_avY])
            # ax1.scatter([Dx],[Dy],marker='o',s=100,facecolors='none',edgecolors='w',label='old')
            ax1.legend(loc=1)

            self.plot_image_simple(ax2, data=bkgd_2D(X, Y) + star2D(X, Y), scale="lin", title="",
                                   units='Background + Gauss (%s)' % self.units)
            ax2.legend(loc=1)

            self.plot_image_simple(ax3, data=sub_image_subtracted - star2D(X, Y), scale="lin", title="",
                                   units='Background+Gauss subtracted image (%s)' % self.units,
                                   target_pixcoords=[new_avX, new_avY])
    ...
    def compute_rotation_angle_hessian(self, deg_threshold=10, width_cut=YWINDOW, right_edge=IMSIZE - 200,
                                       margin_cut=12):
    ...
    def turn_image(self):
        ...
        if not np.isnan(self.rotation_angle):
            self.data_rotated = ndimage.interpolation.rotate(self.data, self.rotation_angle,
                                                             prefilter=parameters.ROT_PREFILTER,
                                                             order=parameters.ROT_ORDER)
            self.stat_errors_rotated = ndimage.interpolation.rotate(self.stat_errors, self.rotation_angle,
                                                                    prefilter=parameters.ROT_PREFILTER,
                                                                    order=parameters.ROT_ORDER)
        if parameters.DEBUG:
            margin = 200
            ...
            self.plot_image_simple(ax1, data=self.data[y0 - parameters.YWINDOW:y0 + parameters.YWINDOW, margin:-margin],
                                   scale="log", title='Raw image (log10 scale)', units=self.units,
                                   target_pixcoords=(self.target_pixcoords[0] - margin, parameters.YWINDOW))
            ax1.plot([0, self.data.shape[0] - 2 * margin], [parameters.YWINDOW, parameters.YWINDOW], 'k-')
            self.plot_image_simple(ax2, data=self.data_rotated[y0 - parameters.YWINDOW:y0 + parameters.YWINDOW,
                                             margin:-margin], scale="log", title='Turned image (log10 scale)',
                                   units=self.units, target_pixcoords=self.target_pixcoords_rotated)
            ax2.plot([0, self.data_rotated.shape[0] - 2 * margin], [parameters.YWINDOW, parameters.YWINDOW], 'k-')
            plt.show()

    def extract_spectrum_from_image(self, spectrum, w=10, ws=(20, 30), right_edge=1800):
        # type: (Spectrum, int, list, int) -> Spectrum
    ...
    def extract_spectrum_from_image_sylvie(self, w=3, ws=[8, 30], right_edge=1800, meanflag=False,
        ...
        """
        self.my_logger.info(
            '\n\tExtracting spectrum from image: spectrum with width 2*%d pixels and background from %d to %d pixels' % (
                w, ws[0], ws[1]))
        ...
        if 0:
            ...
            for x in thex:
                if x < right_edge:
                    ...
                    xprofile_sum_bg[x] = 0.5 * (xprofileUp[x] + xprofileDown[x]) * xprofile_width_inCL[x] / (
                            ws[1] - ws[0])
