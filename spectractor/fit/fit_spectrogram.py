...
class SpectrogramFitWorkspace(FitWorkspace):
    ...
    def set_mask(self, params=None):
        ...
        for k, order in enumerate(self.diffraction_orders):
            ...
            self.spectrogram_simulation.boundaries[order], self.spectrogram_simulation.psf_cubes_masked[order] = self.spectrum.chromatic_psf.get_boundaries(psf_cube_masked)
            self.spectrogram_simulation.psf_cube_sparse_indices[order], self.spectrogram_simulation.M_sparse_indices[order] = self.spectrum.chromatic_psf.get_sparse_indices(psf_cube_masked)
